...
describe('EditAccountModal retired turn-state controls', () => {
  it('does not expose old manual or automatic controls even when legacy settings exist', () => {
    const account = buildOpenAIOAuthParentAccount()
    account.extra = { openai_turn_state_auto: true, openai_turn_state_override: 'old-state' }
    const wrapper = mountModal(account)
    expect(wrapper.find('[data-testid="edit-openai-turn-state-auto"]').exists()).toBe(false)
    expect(wrapper.find('textarea[placeholder="admin.accounts.openai.turnStateOverridePlaceholder"]').exists()).toBe(false)
    wrapper.unmount()
  })
})
