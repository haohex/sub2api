export default {
    accounts: {
      statePool: {
        paste: 'Paste State',
        inputLength: 'Input length: {length}',
        validateReplace: 'Validate and replace',
      imported: 'State validated and replaced',
      importQueued: 'State import queued; it will replace the candidate after validation',
        issuedAt: 'Issued at',
        obtainedAt: 'Acquired at',
        nextProbe: 'Next probe',
        logs: 'Probe and invalidation logs (7 days)',
        loadLogsFailed: 'Failed to load logs; retry',
        noLogs: 'No events yet',
        moreLogs: 'Load older events',
        referenceIP: 'Reference exit IP (queried after probe)',
        ipUnknown: 'Unknown / query failed',
        sent: 'Sent',
        returned: 'Returned',
        notRecorded: 'Not recorded',
        notSent: 'Not sent',
        notReturned: 'Not returned',
        mode_idle: 'Waiting (over 30 minutes left)',
        mode_periodic: 'Periodic probing every 5 minutes',
        mode_continuous: 'Continuous acquisition (1-second interval)',
        mode_manual: 'Manual probing',
        mode_paused: 'Paused',
        event_probe_started: 'Probe started',
        event_probe_result: 'Probe result',
        event_import_started: 'Import validation started',
        event_import_result: 'Import validation result',
        event_invalidation_observed: 'Invalidation signal observed',
        event_invalidated: 'Candidate invalidated',
        event_invalidation_skipped: 'Old candidate changed; newer value retained',
        event_diagnostic: 'Diagnostic event',
        event_paused: 'Probing paused',
        event_idle: 'Waiting resumed',
        event_continuous: 'Continuous acquisition resumed',
        event_periodic: 'Periodic probing resumed',
        event_manual: 'Manual probing',
        replaced: 'Replaced with newer candidate',
        sameState: 'Same state; original expiry retained',
        notNewer: 'Expiry not improved; current candidate retained',
        accountBusy: 'Account is probing; retry later',
        stateRejected: 'Upstream returned another state; input acceptance not confirmed',
        validationIncomplete: 'Validation did not complete; previous state retained',
        invalidFormat: 'Invalid state format or timestamp',
        futureTimestamp: 'State timestamp is in the future',
        accountChanged: 'Account configuration changed; retry',
        concurrentUpdate: 'Candidate changed concurrently',
        encryptedError: 'Encrypted content validation failed; state retained',
        title: 'High-compute State Pool',
        description: 'Candidates are isolated by account and upstream model: Plus / Pro use 292 bytes, Team 332. HTTP SSE acquisition uses fresh proxy connections. Expiry is the earlier one-hour deadline from issuance or acquisition; periodic probing starts at 30 minutes remaining and continuous acquisition at 10 minutes.',
        manageAccounts: 'Manage accounts',
        globalProxy: 'Global rotating probe proxy',
        proxyHint: 'All accounts use this proxy to acquire their own candidates. Normal traffic keeps the account proxy. Changing this proxy preserves valid states.',
        proxyMissing: 'Choose a global rotating proxy first. Probing is paused until configured; direct connections are never used as a fallback.',
        proxyUnavailable: 'The global probe proxy is unavailable or expired. Choose an active proxy.',
        model: 'Model',
        allModels: 'All models',
        search: 'Search account, ID or plan',
        summary: 'Valid {valid} / {total}',
        reload: 'Reload list',
        empty: 'No configured account models match this filter. Add exact model restrictions and enable candidate probing in account settings.',
        unknownPlan: 'Unknown plan',
        expectedLength: 'Expected length {length}',
        remaining: 'Candidate time remaining',
        expiresAt: 'Expires:',
        rawHint: 'State is hidden. Use Copy State to retrieve the current valid value.',
        lastAttempt: 'Last attempt:',
        copy: 'Copy State',
        refresh: 'Probe and refresh',
        expired: 'Expired',
        valid: '{length} healthy candidate',
        disabled: 'Probing disabled',
        invalid: 'Invalid candidate',
        missing: 'No valid candidate',
        running: 'Probing',
        queued: 'Queued',
        stopped: 'Attempt limit reached; manual retry required',
        blocked: 'Paused: check proxy and account status',
        idle: 'Waiting for scheduled renewal',
        downgraded: 'Upstream returned the downgraded model',
        overloaded: 'Upstream reported server_is_overloaded',
        abnormalLength: 'Upstream returned an unexpected state length',
        probeFailed: 'Probe failed; any still-valid previous candidate is retained.',
        loadFailed: 'Failed to load state pool',
        saved: 'Global probe proxy saved',
        saveFailed: 'Failed to save proxy',
        refreshQueued: 'Probe queued. A new candidate replaces the old one only on success.',
        refreshFailed: 'Unable to start probing. Check the account, plan and global proxy.',
        copied: 'State copied',
        copyFailed: 'Unable to copy. The candidate may have expired or become invalid.',
      ...
      openai: {
        ...
        codexTurnStateProbeDesc: 'Use HTTP SSE and the global rotating proxy. Above 30 minutes: manual only; 10–30 minutes: up to 25 attempts per round; final 10 minutes or no valid candidate: continuous acquisition.',
        codexTurnStateGlobalSettings: "Configure the global probe proxy and view candidates in the State Pool",
        ...
        codexTurnStateProbeModelsAutoHint: 'Models are taken automatically from the restriction above. Each actual upstream model ID has its own candidate and renewal timer.',
        codexTurnStateProbeModelsEmpty: 'No finite model restriction is configured above, so there are no models to probe. Add exact models to the whitelist or mapping first.',
        codexTurnStateProbeModelsPlaceholder: 'Enter model IDs separated by commas, e.g. gpt-5.5-codex',
        codexTurnStateProbeModelsHint: 'Select one or more exact upstream model IDs. Candidates are never reused across models.',
        codexTurnStateProbeLoadModels: 'Load models',
        codexTurnStateProbeLoadingModels: 'Loading…',
        ...
        codexTurnStateProbeRetryFailed: 'Failed to reset renewal attempts',
