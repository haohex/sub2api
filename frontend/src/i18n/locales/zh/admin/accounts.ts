export default {
    accounts: {
      statePool: {
        title: '高算力 State 实时池',
        description: '按账号与实际模型隔离候选。Pro / Plus 正常长度为 292，Team 为 332；有效期 1 小时，到期前 5 分钟续期。每模型最多探测 25 次，单次 10 秒，HTTP 未返回 state 时改用新 WS 连接。',
        manageAccounts: '管理账号',
      ...
      openai: {
        ...
        codexTurnStateProbeDesc: "按账号与实际模型隔离候选。Pro / Plus 正常长度为 292，Team 为 332；有效期 1 小时，到期前 5 分钟续期。每模型最多探测 25 次，单次 10 秒，HTTP 未返回 state 时改用新 WS 连接。",
