export default {
    accounts: {
      ...
      openai: {
        ...
        codexTurnStateProbeDesc: '使用本账号 token 通过指定动态代理探测上方模型限制中的模型。收到响应头后立即关闭流；只有长度为 292 的 X-Codex-Turn-State 才会按模型缓存 1 小时，并在到期前 5 分钟自动续期，再注入同一模型的请求。每个模型最多连续尝试 25 次，每次超时 10 秒；日常请求出现降级模型 gpt-5.6-luna、server_is_overloaded 或 312 字节 state 时，提前失效并重新获取。',
