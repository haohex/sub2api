export default {
    accounts: {
      statePool: {
        paste: '粘贴 State',
        inputLength: '输入长度：{length}',
        validateReplace: '校验并替换',
      imported: 'State 已验证并替换',
      importQueued: 'State 导入已排队，验证完成后自动替换',
        issuedAt: '签发时间',
        obtainedAt: '获取时间',
        nextProbe: '下次探测',
        logs: '探测与失效日志（保留 7 天）',
        loadLogsFailed: '日志加载失败，请重试',
        noLogs: '暂无记录',
        moreLogs: '加载更早记录',
        referenceIP: '参考出口 IP（探测后查询）',
        ipUnknown: '未知／查询失败',
        sent: '发送',
        returned: '返回',
        notRecorded: '未记录',
        notSent: '未发送',
        notReturned: '未返回',
        mode_idle: '等待（剩余超过 30 分钟）',
        mode_periodic: '每 5 分钟探测一轮',
        mode_continuous: '持续获取（间隔 1 秒）',
        mode_manual: '手动探测',
        mode_paused: '已暂停',
        event_probe_started: '探测开始',
        event_probe_result: '探测结果',
        event_import_started: '导入校验开始',
        event_import_result: '导入校验结果',
        event_invalidation_observed: '观测到失效信号',
        event_invalidated: '已清理失效候选',
        event_invalidation_skipped: '旧候选已变更，未清理新值',
        event_diagnostic: '诊断事件',
        event_paused: '暂停探测',
        event_idle: '恢复等待',
        event_continuous: '恢复持续获取',
        event_periodic: '恢复周期探测',
        event_manual: '手动探测',
        replaced: '已替换为更新的候选',
        sameState: '相同 State，保留原到期时间',
        notNewer: '有效期未延长，保留当前候选',
        accountBusy: '该账号正在探测，请稍后重试',
        stateRejected: '上游返回不同 State，无法确认输入值被接受',
        validationIncomplete: '上游未正常完成验证，保留旧值',
        invalidFormat: 'State 格式或时间戳无效',
        futureTimestamp: '签发时间异常超前',
        accountChanged: '账号配置已变更，请重试',
        concurrentUpdate: '候选已被其他请求更新',
        encryptedError: '密文校验错误，仅记录，不清理 State',
        title: '高算力 State 实时池',
        description: '按账号与实际模型隔离，Plus / Pro 正常长度 292，Team 为 332。仅 HTTP SSE 获取，每次新建代理连接；以签发和获取时间中较早的一小时期限为准，剩余 30 分钟内周期探测，最后 10 分钟持续获取。',
        manageAccounts: '管理账号',
        globalProxy: '全局动态探测代理',
        proxyHint: '所有账号共用此代理获取各自的候选，正常请求仍走账号代理。更换代理不会清空有效 state。',
        proxyMissing: '请先选择全局动态代理；配置前暂停探测，不会直连兜底。',
        proxyUnavailable: '全局探测代理不可用或已过期，请重新选择。',
        model: '模型',
        allModels: '全部模型',
        search: '搜索账号名称、编号或套餐',
        summary: '当前有效 {valid} / {total}',
        reload: '更新列表',
        empty: '此筛选下没有配置模型的账号。请在账号管理中设置精确模型限制，并开启候选探测。',
        unknownPlan: '套餐未识别',
        expectedLength: '正常长度 {length}',
        remaining: '候选剩余有效期',
        expiresAt: '到期时间：',
        rawHint: 'State 默认隐藏，点击“复制 State”获取当前有效值。',
        lastAttempt: '最近探测：',
        copy: '复制 State',
        refresh: '立即探测刷新',
        expired: '已过期',
        ...
        running: '探测中',
        queued: '等待探测',
        stopped: '失败达到上限，等待手动重试',
        blocked: '探测暂停：检查代理和账号状态',
        idle: '等待定时续期',
        downgraded: '上游返回降级模型',
        overloaded: '上游报告 server_is_overloaded',
        abnormalLength: '上游返回异常长度的 state',
        probeFailed: '本轮探测失败；仍有效的旧候选继续保留。',
        loadFailed: '加载状态池失败',
        saved: '全局探测代理已保存',
        saveFailed: '保存代理失败',
        refreshQueued: '已加入后台探测队列；成功后替换，失败保留有效旧值。',
        refreshFailed: '无法启动探测，请检查账号、套餐与全局代理。',
        copied: 'State 已复制',
        copyFailed: '无法复制，候选可能已过期或失效。',
      ...
      openai: {
        ...
        codexTurnStateProbe: 'Codex turn-state 候选探测',
        codexTurnStateProbeDesc: '仅使用 HTTP SSE 和全局动态代理获取候选。剩余超过 30 分钟仅手动探测，10–30 分钟每轮最多 25 次，最后 10 分钟或无有效候选时持续获取。',
        codexTurnStateGlobalSettings: "在状态池页面配置全局动态代理并查看候选",
        codexTurnStateProbeProxy: '探测代理',
        codexTurnStateProbeProxyHint: '这里只用于获取候选 state；账号日常请求仍使用账号自己的代理。',
        codexTurnStateProbeModels: '探测模型',
        codexTurnStateProbeModelsAutoHint: '探测模型自动取自上方的模型白名单或映射；每个实际上游模型 ID 都独立缓存候选 state 和计时。',
        codexTurnStateProbeModelsEmpty: '上方没有配置有限的模型限制，因此没有可探测的模型。请先在白名单或映射中添加精确模型。',
        codexTurnStateProbeModelsPlaceholder: '输入模型 ID，逗号分隔，例如 gpt-5.5-codex',
        codexTurnStateProbeModelsHint: '选择或输入一个或多个精确的上游模型 ID；候选 state 不跨模型复用。',
        codexTurnStateProbeLoadModels: '加载模型',
        codexTurnStateProbeLoadingModels: '加载中…',
        codexTurnStateProbeProxyRequired: '请选择探测代理',
        codexTurnStateProbeModelsRequired: '请至少选择或输入一个模型',
        codexTurnStateProbeLoadModelsFailed: '加载账号模型失败',
        codexTurnStateProbeRenewalFailed: '候选 state 已连续续期失败 25 次',
        codexTurnStateProbeRenewalFailedDesc: '以下模型需要手动继续尝试：',
        codexTurnStateProbeRetry: '继续续期',
        codexTurnStateProbeRetryStarted: '已重置失败次数，后台将继续续期',
        codexTurnStateProbeRetryFailed: '重置续期失败次数失败',
