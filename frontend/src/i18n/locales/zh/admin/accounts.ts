export default {
    accounts: {
      ...
      openai: {
        ...
        codexTurnStateProbeDesc: '使用本账号 token 通过指定动态代理探测上方模型限制中的模型。收到响应头后立即关闭流；只有长度为 292 的 X-Codex-Turn-State 才会按模型缓存 1 小时，并在到期前 5 分钟自动续期，再注入同一模型的请求。',
        ...
        codexTurnStateProbeRenewalFailed: '候选 state 已连续续期失败 50 次',
